...
siku_dir=str(sikufenci.__file__).replace('\__init__.py','').replace('/__init__.py','')
# print(siku_dir)
cpu_num = cpu_count() # 自动获取最大核心数目
print('当前计算机共有:'+str(cpu_num)+'个核心')
...
def readfile(filename):#从外部读取文件
    ...
    '''
    f = open(siku_dir+'/'+filename,'r',encoding='utf-8')
    data = []
    sentence = []
    label= []
    for line in f:
...
def get_test(inputpath,datapath,max_seq_length):
        ...
        for j in test_list:
            ...
            if len(j)<max_seq_length-2:
               ...
               test.extend(clip_list(j,max_seq_length-2))
        # filename=os.path.abspath('wordseg_data')
        # print(filename)
        with open(siku_dir+'/'+datapath+'/test.txt','w',encoding='utf-8') as fp:
            for i in test:
                for j in i:
                    fp.write(str(j)+' '+'S'+'\n')
                fp.write('\n')
...
def get_train(inputpath,datapath):
        f=open(siku_dir+'/'+datapath+'/test.txt','r',encoding='utf-8')
        content=f.read()
        # filename=os.path.abspath('wordseg_data')
        with open(siku_dir+'/'+datapath+'/train.txt','w',encoding='utf-8') as fp:
            fp.write(content)
...
def get_dev(inputpath,datapath):
        ...
        f=open(siku_dir+'/'+datapath+'/test.txt','r',encoding='utf-8')
        content=f.read()
        with open(siku_dir+'/'+datapath+'/valid.txt','w',encoding='utf-8') as fp:
            fp.write(content)
...
def get_seq(inputpath,max_seq_length):
   f=open(inputpath,'r',encoding='utf-8')
   sentence=f.readlines()
   result=[]
   for i in sentence:
        if len(i)<max_seq_length-2:
           result.append(i)
        else:
            result.extend(clip_list(i,max_seq_length-2))  #防止用户的输入序列过长
   return result



def generate(inputpath,datapath,max_seq_length):
    get_test(inputpath,datapath,max_seq_length)
...
def fun(path):
    ...
    for root, dirs, files in os.walk(path):
        for i in files:
            eachpath = str(i)
            fileArray.append(root+'/'+eachpath)
    # print(fileArray)
    return fileArray
...
def process_txt(raw_path,output,max_seq_length=512,eval_batch_size=30):
    ...
    generate(raw_path,filepath,max_seq_length)
    task_name='ner'
    dataname=filepath+'/train.txt'
    valid_txt=filepath+'/valid.txt'
    dev=filepath
    # max_seq_length=500
    bert_model=siku_dir+'/'+'pretrain_models/sikuroberta_vocabtxt'
    output_dir=siku_dir+'/'+'train_fenci_sikuroberta_vocabtxt'
    # tempoutputpath='temp_data/'

    datalist=get_seq(raw_path,max_seq_length)
    token=[]
    for i in datalist:
        temp=[]
        for j in i:
    ...
    label=[]
    sentence=[]
    valid_data=readfile(filepath+'/valid.txt')
    ...
    if torch.cuda.is_available():
        model.load_state_dict(torch.load(output_model_file,map_location='cuda'),False)
    else:
    ...
    generate_result(alltokens, y_true, y_pred, os.path.join(siku_dir+'/'+output, "labeled_results.txt"))

def get_output(content):
    output_text=content.replace('B','').replace('I','').replace('E','/').replace('S','/')
    return output_text

def fenci(outputpath,file):
    test=[]
    list3=[]
    predict=[]
    tokens=[]
    f1=open(siku_dir+'/'+'outputdata/labeled_results.txt','r',encoding='utf-8')
    f2=open(siku_dir+'/'+'wordseg_data/test.txt','r',encoding='utf-8')
    list1=f1.readlines()
    for i in list1:
        i=re.sub('\t','',i)
        list3.append(i)
    ...
    for i in range(0,len(combine)):
        ...
        for j in range(0,len(combine[i][0])):
            try:
                str1=combine[i][0][j]+combine[i][1][j]
                temp.append(str1)
                str2=''.join(temp)
            except:
                error_list.append(combine[i][0])
                # error_list=list(set(error_list))
                # with open(outputpath+'/'+'转化的失败句子.txt','a+',encoding='utf-8')as fp:
                #     fp.write(combine[i][0]+'\n')
        combines.append(str2)
    with open(outputpath+'/'+file+'_result.txt','w',encoding='utf-8')as fp:
        for i in combines[0:-1]:
            lines=get_output(i)
            fp.write(lines+'\n')
    error_list=list(set(error_list))
    with open(outputpath+'/'+'转化的失败句子.txt','a+',encoding='utf-8')as fp:
...
def TCfenci_all(raw_path,resultpath,max_seq_length,eval_batch_size):
    filearray=fun(raw_path)
    print(filearray)
    for i in filearray:
       process_txt(i,output='outputdata',max_seq_length=max_seq_length,eval_batch_size=eval_batch_size)
       fenci(resultpath,i.split('/')[-1].replace('.txt',''))
...
if __name__ == "__main__":
    ...
    # get_seq('data')
    TCfenci_all(raw_path='data',resultpath='final_result',max_seq_length=128,eval_batch_size=3)
    # fun('data')